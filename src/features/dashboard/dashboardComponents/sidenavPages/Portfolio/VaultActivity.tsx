...
interface AllVaultsActivityProps {
  masterVaultPda?: string
  copierVaultsPdas: string[]
}

export function AllVaultsActivity ({ masterVaultPda, copierVaultsPdas }: AllVaultsActivityProps) {
  const allVaults = [
    ...(masterVaultPda ? [masterVaultPda] : []),
    ...copierVaultsPdas
  ]
  
  const { activities: fetchedActivities, isLoading } = useAllVaultActivities(allVaults, 15)
  const displayActivities = fetchedActivities.map(formatVaultActivity)
