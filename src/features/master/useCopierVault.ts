...
export const useCopierVault = () => {
  ...
  const createCopierVault = useCallback(
    async (params: CopierVaultParams) => {
      ...
      try {
        ...
        if (existingVault) {
          console.log(
            "Copier vault already exists on-chain, syncing with backend...",
          );
          const response = await authFetch<{
            success: boolean;
            message?: string;
            data: { vaultPda: string };
          }>("/api/vaults/copier_vault/sync", {
            method: "POST",
            body: JSON.stringify({
          ...
          if (!response.success) {
            throw new Error(
              response.message || "Failed to sync existing vault with backend",
            );
          }
        ...
        console.log(
          "Fetching master vault account details for:",
          masterVaultPubkey.toString(),
        );

        let masterVaultAccount;
        try {
          masterVaultAccount =
            await program.account.masterExecutionVault.fetch(masterVaultPubkey);
        } catch (fetchErr: unknown) {
          console.error("Failed to fetch master vault account:", fetchErr);
          throw new Error(
            `Master vault account ${masterVaultPubkey.toString()} not found on-chain. Ensure the trader has initialized their vault.`,
          );
        }
        ...
        console.log(
          "Initializing copier vault on-chain...",
          copierVaultPda.toString(),
        );
        ...
        try {
          ...
          if (
            ...
            console.log(
              "Creation transaction was already processed. Continuing to sync.",
            );
          } else {
            throw txErr;
          }
