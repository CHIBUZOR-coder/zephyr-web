...
type JupiterQuote = {
  ...
  routePlan: Array<{
    swapInfo: {
      ammLabel: string;
      tokenFees: Array<{ mint: string; amount: string }>;
    };
    percent: number;
  }>;
  slippageBps: number;
    ...
    try {
      ...
      if (logs) {
        ...
        if (
          ...
          logsStr.includes("insufficient funds") ||
          logsStr.includes("0x1787")
        ) {
          const match = logsStr.match(
        ...
        if (
          logsStr.includes("depositBelowMinimum") ||
          logsStr.includes("Minimum deposit")
        ) {
        ...
        if (
          ...
          logsStr.includes("Blockhash not found")
        ) {
...
export const useVaultOperations = () => {
  ...
  const depositToCopierVault = useCallback(
    async (vaultPda: string, amountSol: number) => {
        ...
        try {
          ...
          if (txErr instanceof Error) {
            ...
            if (
              ...
              console.log(
                "Deposit transaction was already processed. Treating as success.",
              );
            } else {
              throw txErr;
            }
  ...
  const withdrawFromCopierVault = useCallback(
    async (vaultPda: string, amountSol: number) => {
      if (!program || !publicKey)
        throw new Error("Program or wallet not initialized");
      ...
      try {
        ...
        console.log(
          `Withdrawing ${amountSol} SOL from copier vault ${vaultPda}...`,
        );
  ...
  const depositToMasterVault = useCallback(
    async (amountSol: number) => {
      ...
      try {
        ...
        console.log(
          `Depositing ${amountSol} SOL into master vault ${masterVaultPda.toBase58()}...`,
        );
  ...
  const withdrawFromMasterVault = useCallback(
    async (amountSol: number) => {
      if (!program || !publicKey)
        throw new Error("Program or wallet not initialized");
      ...
      try {
        ...
        console.log(
          `Withdrawing ${amountSol} SOL from master vault capital ${masterVaultPda.toBase58()}...`,
        );
  ...
  const claimPerformanceFees = useCallback(
    async (amountSol: number) => {
      if (!program || !publicKey)
        throw new Error("Program or wallet not initialized");
      ...
      try {
        ...
        console.log(
          `Claiming ${amountSol} SOL in fees from master vault ${masterVaultPda.toBase58()}...`,
        );
  ...
  const getMasterVaultBalances = useCallback(async () => {
    ...
    console.log(
      `[BalanceCheck] Total: ${vaultLamports / LAMPORTS_PER_SOL} SOL, Rent: ${rentExemptMin / LAMPORTS_PER_SOL} SOL, Spendable: ${spendableLamports / LAMPORTS_PER_SOL} SOL`,
    );
    ...
    async (params: {
      ...
      try {
        ...
        const vaultAccountInfo =
          await connection.getAccountInfo(masterVaultPda);
        const rentExemptMin = vaultAccountInfo
          ? await connection.getMinimumBalanceForRentExemption(
              vaultAccountInfo.data.length,
            )
        ...
        const volumeFeeBps = Number(masterVaultAccount.volumeFeePercent);
        const estimatedPlatformFee = Math.floor(
          (params.amountIn * volumeFeeBps * 0.2) / 10000, // 20% of volume fee goes to platform
        );
        const totalRequired = params.amountIn + estimatedPlatformFee;

        if (spendableLamports < totalRequired) {
          throw new Error(
            `Vault has insufficient funds. ` +
              `Need ${(totalRequired / LAMPORTS_PER_SOL).toFixed(6)} SOL ` +
              `(${(params.amountIn / LAMPORTS_PER_SOL).toFixed(6)} trade + ` +
              `${(estimatedPlatformFee / LAMPORTS_PER_SOL).toFixed(6)} fee), ` +
              `available: ${(spendableLamports / LAMPORTS_PER_SOL).toFixed(6)} SOL.`,
          );
        }
        ...
        if (isMockNetwork) {
          ...
          const quoteRes = await authFetch<{
            success: boolean;
            data: JupiterQuote;
            error?: string;
          }>(quoteUrl);
          ...
          if (!quoteRes.success || !quoteRes.data) {
            throw new Error(
              `Jupiter quote failed: ${quoteRes.error || "Unknown error"}`,
            );
          }
          ...
          console.log(
            "Fetching Jupiter swap instructions via Backend Proxy...",
          );

          const swapRes = await authFetch<{
            success: boolean;
          ...
          if (!swapRes.success || !swapRes.data) {
            throw new Error(
              `Jupiter swap-instructions failed: ${swapRes.error || "Unknown error"}`,
            );
          }
          ...
          jupiterSwapAccounts = swapIx.accounts.map((a: ApiAccount) => ({
            ...
            isWritable: a.isWritable,
            isSigner:
              a.pubkey === masterVaultPda.toBase58() ? false : a.isSigner,
          }));
  ...
  const initializeRiskConfig = useCallback(async () => {
    ...
    try {
      ...
      const tx = await (
        program.methods.initializeRiskConfig(
  ...
  const updateRiskConfig = useCallback(async () => {
    ...
    try {
      ...
      const tx = await (
        program.methods.updateRiskConfig(
  ...
  const updateCopierRiskParams = useCallback(
    async (
      ...
      try {
        ...
        const [copierVaultPda] = PublicKey.findProgramAddressSync(
          [Buffer.from("vault"), publicKey.toBuffer(), masterVault.toBuffer()],
          program.programId,
        );
        ...
        if (!copierVaultAccount) {
          console.log(
            "Copier vault not initialized on-chain, attempting auto-initialization...",
          );
          ...
          try {
            const masterVaultAccount =
              await program.account.masterExecutionVault.fetch(masterVault);
            ...
            if (errMsg.includes("discriminator")) {
              console.log(
                "Master vault was created with old program version - need to reinitialize",
              );
              isOldProgram = true;
            }
          }
          ...
          try {
            await (
              program.methods.initializeCopierVault(
                {
            ...
            if (
              initErrMsg.includes("already been used") ||
              initErrMsg.includes("0x0")
            ) {
              console.log(
                "Copier vault already initialized, continuing to update",
              );
            } else if (
              isOldProgram ||
              initErrMsg.includes("discriminator") ||
              initErrMsg.includes("AccountNotInitialized")
            ) {
        ...
        const tx = await (
          program.methods.updateRiskParams(
            {
              maxLossPct: newParams.maxLossPct,
              maxTradeSizePct: newParams.maxTradeSizePct,
              maxDrawdownPct: newParams.maxDrawdownPct,
            },
        ...
        try {
          await authFetch(
            `/api/vaults/copier/${copierVaultPda.toBase58()}/risk-params`,
            {
              method: "PUT",
          ...
          console.warn(
            "Failed to update risk params in database (on-chain update succeeded):",
            dbErr,
          );
        }
        ...
        if (errMsg.includes("discriminator")) {
          friendlyError =
            "This copier relationship was created with an old version of the program. Please re-copy the master vault.";
        }
